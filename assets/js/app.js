...
window.addEventListener("load", function() {
    let logoShield = document.querySelectorAll(".logoShield")
    let logoItem = document.querySelectorAll(".logoItem")
    let arrowStart = document.querySelectorAll("#arrowStart")

    logoItem.forEach(function (item) {
        setTimeout(function() {
            item.style.fill = "#FDD762"
            item.style.transform = "translateY(0)"
        }, 1000)

        item.addEventListener("mouseenter", function( event ) {
            event.target.style.transform = "translateY(-20%)"
            setTimeout(function() {
    ...
    logoShield.forEach(function (item) {
        setTimeout(function() {
            item.style.fill = "#FDD762"
        }, 1500)

        let logoShieldID = document.getElementById("logoShield")
        setInterval(function (){
        }, 1500)

    }, false)

    arrowStart.forEach(function (item) {
        setTimeout(function() {
            item.style.transform = "translateY(0) rotate(90deg)"
        }, 1500)
    }, false)
})


//ANIMATION SLIDE (OPEN WEB SITE)

let buttonNext = document.getElementById("arrowStart")
let slide = document.getElementById("slide")
let container = document.getElementById("container")
let home = document.getElementById("home")

buttonNext.addEventListener("mouseenter", function( event ) {
    buttonNext.style.transform = "translateY(0%) rotate(-90deg)"
}, false);
buttonNext.addEventListener("mouseleave", function( event ) {
    buttonNext.style.transform = "translateY(0%) rotate(90deg)"
}, false);
...
setTimeout(function() {
    new TypeIt("#slogan", {
        strings: "Réseau de distribution commercial.",
        speed: 50,
